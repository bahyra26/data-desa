    ...
    <section class="card form-card">
        <form action="{{ route('desa.update', $desa) }}" method="POST">
            ...
            <div class="field">
                ...
                <select id="kecamatan_id" name="kecamatan_id">
                    ...
                    @foreach ($kecamatans as $kecamatan)
                        <option value="{{ $kecamatan->id }}" @selected(old('kecamatan_id', $desa->wilayah->kecamatan_id) == $kecamatan->id)>{{ $kecamatan->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="field">
                ...
                <textarea id="alamat_kantor" name="alamat_kantor" placeholder="Contoh: Kantor Desa ..., Kecamatan ..., Kabupaten Pasuruan">{{ old('alamat_kantor', $desa->alamat_kantor) }}</textarea>
            ...
            <div class="field">
                ...
                <input type="number" id="jumlah_penduduk" name="jumlah_penduduk" min="0" value="{{ old('jumlah_penduduk', $desa->jumlah_penduduk) }}">
                @error('jumlah_penduduk') <div class="error">{{ $message }}</div> @enderror
            </div>

            <div class="field">
                <label for="luas_wilayah">Luas Wilayah (km2)</label>
                <input type="number" step="0.01" id="luas_wilayah" name="luas_wilayah" min="0" value="{{ old('luas_wilayah', $desa->luas_wilayah) }}">
                @error('luas_wilayah') <div class="error">{{ $message }}</div> @enderror
            </div>

            <div class="actions">
                <button type="submit" class="button button-primary">Update</button>
                <a href="{{ route('desa.show', $desa) }}" class="button button-secondary">Batal</a>
            </div>
        </form>
    </section>
@endsection
...
@push('scripts')
    <script>
        ...
        const selectedKecamatan = '{{ old('kecamatan_id', $desa->wilayah->kecamatan_id) }}';
        const selectedWilayah = '{{ old('wilayah_id', $desa->wilayah_id) }}';
